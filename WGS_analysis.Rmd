...
BT16_coverage <- read_tsv("./data/WGS_analysis/average_coverage_10kb_BT16.bed")

BT16_coverage_filt <- BT16_coverage %>%
  filter(Chr %in% c(paste0("chr", 1:22)))


BT16_coverage_filt$Chr <- factor(BT16_coverage_filt$Chr, levels = paste0("chr", 1:22))
BT16_coverage_filt$color <- ifelse(as.numeric(factor(BT16_coverage_filt$Chr)) %% 2 == 0, "black", "gray")

BT16_coverage_filt <- BT16_coverage_filt %>%
  mutate(RegionLength = End - Start)
...
start_positions <- BT16_coverage_filt %>%
  group_by(Chr) %>%
  filter(row_number() == 1) %>%
  select(Chr, CumulativeSum)
...
chromosome_midpoints <- BT16_coverage_filt %>%
  group_by(Chr) %>%
  summarise(Midpoint = median(CumulativeSum))


BT16_coverage_plot <- ggplot(BT16_coverage_filt, aes(x = CumulativeSum, y = Coverage)) +
  geom_line(color = BT16_coverage_filt$color) +  # Line plot for coverage
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +  # Rotate x labels if needed
  labs(y = "Coverage", x = NULL) +
  theme_minimal() +
  theme(
...
ggsave(filename = "./plots/WGS_analysis/BT16_coverage_uniformity.tiff", plot = BT16_coverage_plot, 
       width = 10, height = 6, dpi = 300)
...
BT16_coverage_filt_chr22 <- BT16_coverage_filt %>%
  filter(BT16_coverage_filt$Chr == "chr22")



BT16_coverage_filt_chr22_SMARCB1 <- BT16_coverage_filt_chr22 %>%
  filter(BT16_coverage_filt_chr22$Start >23500000 & BT16_coverage_filt_chr22$Start <24250000)

SMARCB1_BT16 <- ggplot(BT16_coverage_filt_chr22_SMARCB1, aes(x = Start, y = Coverage)) +
  geom_line() +
  labs(x = "Position", y = "Coverage") +
  theme_minimal() +
  theme(
  ...
  ylim(0, 60) +
  geom_hline(yintercept = 0, color = "black", size = 0.5) + 
  geom_vline(xintercept = 23500000, color = "black", size = 0.5) +
  geom_rect(aes(xmin = 24129153, xmax = 24180196, ymin = 1, ymax = 4), 
            fill = "red", alpha = 0.5) +
  geom_text(aes(x = (24100000 + 24180196) / 2, y = 5, label = "SMARCB1"), 
            color = "black", size = 3, fontface = "plain")
...
ggsave(filename = "./plots/WGS_analysis/BT16_SMARCB1_coverage.tiff", plot = SMARCB1_BT16, 
       width = 10, height = 6, dpi = 300)
