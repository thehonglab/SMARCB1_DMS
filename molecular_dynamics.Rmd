...
for energy_type in "${energy_types[@]}"; do
    ...
    gawk -F'\t' -v energy_type="$energy_type" '
    BEGIN { print "Mutation\tEnergy_Type\tMean_ddG\tStdev\tSE\tt_statistic\tn" }
    NR > 1 {
        if ($6 != "NA" && $7 != "NA") {
            ddg = $6 - $7
            i315r_vals[n1++] = ddg
    ...
    END {
        ...
        se1 = stdev1 / sqrt(n1)
        t1 = mean1 / se1
        printf "I315R\t%s\t%.4f\t%.4f\t%.4f\t%.4f\t%d\n", energy_type, mean1, stdev1, se1, t1, n1
        ...
        se2 = stdev2 / sqrt(n2)
        t2 = mean2 / se2
        printf "W281P\t%s\t%.4f\t%.4f\t%.4f\t%.4f\t%d\n", energy_type, mean2, stdev2, se2, t2, n2
    }' ${energy_type,,}_differences.txt | \
    awk -F'\t' 'NR==1 { print $0 "\tDF\tp_value\tCohen_d\tCI_lower\tCI_upper"; next }
    {
        t_val = $6
        n_val = $7
        mean_val = $3
        ...
        stdev_val = $4
        df_val = n_val - 1
        pyfile = "/tmp/calc_stats_" NR ".py"
        outfile = "/tmp/calc_stats_" NR ".out"
        print "from scipy.stats import t as tdist" > pyfile
        print "df=" df_val >> pyfile
        print "t_val=" t_val >> pyfile
        print "n=" n_val >> pyfile
        print "mean=" mean_val >> pyfile
        print "se=" se_val >> pyfile
        print "stdev=" stdev_val >> pyfile
        print "pval = 2 * tdist.sf(abs(t_val), df=df)" >> pyfile
        print "t_crit = tdist.ppf(0.975, df=df)" >> pyfile
        print "ci_lower = mean - t_crit * se" >> pyfile
        print "ci_upper = mean + t_crit * se" >> pyfile
        print "cohen_d = mean / stdev" >> pyfile
        print "print(f\"{df}\\t{pval:.6e}\\t{cohen_d:.4f}\\t{ci_lower:.4f}\\t{ci_upper:.4f}\")" >> pyfile
        close(pyfile)
        system("python3 " pyfile " > " outfile)
        getline result < outfile
        close(outfile)
        print $0 "\t" result
    }' > ${energy_type,,}_statistics_summary.txt
...
echo -e "Mutation\tEnergy_Type\tMean_ddG\tStdev\tSE\tt_statistic\tn\tDF\tp_value\tCohen_d\tCI_lower\tCI_upper" > combined_statistics_summary.txt
