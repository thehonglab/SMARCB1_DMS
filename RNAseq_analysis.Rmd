...
plot3 <- ggplot(pca_samples,
  ...
  geom_point(size=4) +
  scale_color_manual(values = c("WT" = "#000000",      # Black
                               "I315I" = "#E69F00",     # Orange
                               "W281P" = "#56B4E9",     # Sky blue
                               "I315R" = "#009E73",     # Bluish green
                               "W281*" = "#F0E442",     # Yellow
                               "I315*" = "#CC79A7")) +  # Reddish purple
  theme(
...
normalized_counts_df3 <-  rownames_to_column(as.data.frame(normalized_counts_df2))
...
residue_WTvW281P$Mutant <- relevel(residue_WTvW281P$Mutant, ref = "zW281P")
...
res_WTvW281P_df$ENSEMBL <- sub('\\.[0-9]*$', '', res_WTvW281P_df$ENSEMBL)

write_xlsx(res_WTvW281P_df, "./results/rna_analysis/WTvW281P_all_genes.xlsx")
...
keep = rowSums(counts(dds_i315i)) >= 10
...
keep = rowSums(counts(dds_i315r)) >= 10
dds_i315r = dds_i315r[keep,]
...
file_names_315Iv315R = residue_315Iv315R$Filename

file_names_315Iv315R = file.path("./data/rnaseq_analysis/quants", file_names_315Iv315R)

names(file_names_315Iv315R) = residue_315Iv315R$Filename

tx_data_315Iv315R = tximport(file_names_315Iv315R,
                   type = c("salmon"),
                   tx2gene = t2g,
                   abundanceCol = "TPM",
                   countsCol = "NumReads",
...
residue_315Iv315R$Mutant <- factor(residue_315Iv315R$Mutant)

residue_315Iv315R$Mutant <- relevel(residue_315Iv315R$Mutant, ref = "zI315R")

levels(residue_315Iv315R$Mutant)

dds_315Iv315R = DESeqDataSetFromTximport(tx_data_315Iv315R,
                               colData = residue_315Iv315R,
                               design = ~ Mutant)

dds_315Iv315R <- estimateSizeFactors(dds_315Iv315R)


keep = rowSums(counts(dds_315Iv315R)) >= 10
dds_315Iv315R = dds_315Iv315R[keep,]

dds_diff_315Iv315R <- DESeq(dds_315Iv315R)

res_315Iv315R <- results(dds_diff_315Iv315R)

res_315Iv315R_df <- as.data.frame(res_315Iv315R)

res_315Iv315R_df$ENSEMBL <- rownames(res_315Iv315R_df)

res_315Iv315R_df$ENSEMBL <- sub('\\.[0-9]*$', '', res_315Iv315R_df$ENSEMBL)


write_xlsx(res_315Iv315R_df, "./results/rna_analysis/I315IvI315R_all_genes.xlsx")
...
I315IvI315X_up <- I315IvI315X_up[order(-I315IvI315X_up$log2FoldChange),] 

I315IvI315X_up_top50 <- head(I315IvI315X_up, 25)

I315IvI315X_up_SMARCB1 <- I315IvI315X_up %>%
  filter(I315IvI315X_up$ENSEMBL == "SMARCB1")

I315IvI315X_up_top50 <- rbind(I315IvI315X_up_top50, I315IvI315X_up_SMARCB1)

all_heatmap_input <- normalized_counts_df2 %>%
  filter(normalized_counts_df2$rowname %in% I315IvI315X_up_top50$ENSEMBL)

all_heatmap_input <- all_heatmap_input[order(-all_heatmap_input$G401_P16_STT21_MRT00_SEQ61_WT_NOV5.quant.sf),] 

gene_names_all <- all_heatmap_input$rowname



#all_heatmap_input <- all_heatmap_input[,c(7,13,5,11,6,12,2,8,3,9,4,10)]


#colnames(all_heatmap_input) <- c("WT_R1","WT_R2","W281P_R1","W281P_R2","W281X_R1","W281X_R2","I315I_R1","I315I_R2","I315R_R1","I315R_R2","I315X_R1","I315X_R2")

all_heatmap_input <- all_heatmap_input[,c(10,4,9,3,8,2,12,6,11,5,13,7)]

colnames(all_heatmap_input) <- c("I315X_R2","I315X_R1","I315R_R2","I315R_R1","I315I_R2","I315I_R1","W281X_R2","W281X_R1","W281P_R2","W281P_R1","WT_R2","WT_R1")

all_heatmap_input <- as.data.frame(all_heatmap_input)


all_heatmap_input <- all_heatmap_input[order(-all_heatmap_input$WT_R1),] 
...
plot <- pheatmap(all_heatmap_input, labels_row = gene_names_all, cluster_cols=F, cluster_rows=F, fontsize_row = 18, fontsize_col=15)

plot


ggsave("/Users/pedadministrator/Library/CloudStorage/GoogleDrive-gcooper.gene@gmail.com/My Drive/Hong Lab Project Folders (1)/Results GC/17-SMARCB1_MITEseq2/RNAseq/all_heatmap_top25_up.tiff", plot = plot, width = 5, height = 10, dpi=600)
