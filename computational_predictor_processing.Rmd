...
tiff("./plots/computational_correlations/CADD_v_DMS_NMS.tiff", width = 10, height = 5, units = "in", res = 300)
...
NS_only <- merged_df4 %>%
  filter(merged_df4$Mutation_type == "S"| merged_df4$Mutation_type == "Stop")

CADD_DMS_NS <- na.omit(NS_only[,c(1,2,5,6)])

colors <- ifelse(CADD_DMS_NS$Mutation_type == "S", "black", "red")

plot(CADD_DMS_NS$PHRED, CADD_DMS_NS$all_reps_zscore,
     col = colors, pch = 19,  # pch = 19 for solid circles
     xlab = "PHRED", ylab = "All Reps Z-Score")

lm_model <- lm(all_reps_zscore ~ PHRED, data = CADD_DMS_NS)

r_squared <- summary(lm_model)$r.squared

text(x = max(CADD_DMS_NS$PHRED), y = max(CADD_DMS_NS$all_reps_zscore), 
     labels = paste("R^2 = ", round(r_squared, 2)), pos = 2, cex = 1.2, col = "blue")
