...
colnames(cadd_data_exons)[colnames(cadd_data_exons) == "#Chrom"] <- "Chrom"
...
cadd_data_no_duplicates <- cadd_data_exons %>%
  ...
  summarise(
    across(-PHRED, ~ dplyr::first(.), .names = "{.col}"), # Keep the first instance for all columns except PHRED
...
cadd_percentile_inclusive <- mean(cadd_data_exons_missense$PHRED <= cadd_r377h_score, na.rm = TRUE)
...
cadd_amino_acid_rank <- ggplot(cadd_data_exons_missense, aes(y = reorder(Amino_Acid_Alt, PHRED, FUN = median),
                                                          x = PHRED)) +
  geom_jitter(size = 1.5, color = "steelblue", alpha = 0.6, height = 0.2) +
  stat_summary(fun = median, geom = "crossbar", color = "red", height = 0.5, size = 0.5) +
...
revel_data_exons <- revel_data_exons %>%
  left_join(cadd_data_exons_unique %>% dplyr::select(Pos, Codon, Site), by = "Pos")
...
revel_data_exons <- revel_data_exons %>%
  left_join(cadd_data_exons_unique_codon %>% dplyr::select(Codon, Ref_Codon), by = "Codon")
...
revel_data_no_duplicates <- revel_data_exons %>%
  group_by(AA_Substitution) %>%
  summarise(
    across(-REVEL, ~ dplyr::first(.), .names = "{.col}"), # Keep the first instance of all columns
...
revel_r377h_score <- 0.868

revel_percentile <- mean(revel_data_no_duplicates$REVEL < revel_r377h_score, na.rm = TRUE)
...
alpha_missense <- alpha_missense %>%
  mutate(
    ...
    Amino_Acid_Alt = str_sub(protein_variant, start = -1),
    Amino_Acid_Ref = str_sub(protein_variant, start = 1, end = 1)# Extract the last letter
  )
...
am_r377h_score <- 0.9996


am_percentile <- mean(alpha_missense$am_pathogenicity < am_r377h_score, na.rm = TRUE)
...
SMARCB1_sift <- read_tsv("./data/computational_predictors/SMARCB1_SIFT4g_GRCh38.83.tsv")


SMARCB1_v1_sift <- SMARCB1_sift %>%
  filter(SMARCB1_sift$Transcript_id == "ENST00000263121")

SMARCB1_v1_cds_sift <- SMARCB1_v1_sift %>%
  filter(SMARCB1_v1_sift$Region == "CDS")

SMARCB1_v1_cds_sift_nostop <- SMARCB1_v1_cds_sift %>%
  filter(SMARCB1_v1_cds_sift$New_amino_acid != "*")

SMARCB1_v1_cds_sift_nostop <- SMARCB1_v1_cds_sift_nostop %>%
  filter(SMARCB1_v1_cds_sift_nostop$Ref_amino_acid != "*")

SMARCB1_v1_cds_sift_nostop_nosilent <- SMARCB1_v1_cds_sift_nostop %>%
  filter(SMARCB1_v1_cds_sift_nostop$Ref_amino_acid != SMARCB1_v1_cds_sift_nostop$New_amino_acid)




write_tsv(SMARCB1_v1_cds_sift_nostop_nosilent, "./data/computational_predictors/SIFT_SMARCB1.tsv")
